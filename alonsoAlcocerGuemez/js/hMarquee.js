...
!(function($) {
  ...
  function bindStyles(opt) {
    ...
    if (!$('#m-marquee-style').length) {
      styleText =
        '.m-marquee { position: relative; overflow: hidden; -webkit-transition: height .2s ease-in; transition: height .2s ease-in; }' +
      ...
      $('head').append(
        $('<style id="m-marquee-style"></style>').text(styleText)
      );
    }
    ...
    opt.leftItem &&
      ...
      (backgroundText +=
        '.m-marquee.' +
        opt.marqueeId +
        '.m-marquee-left .m-marquee-left-item:before { background-image: url("' +
        opt.leftItemImg +
        '"); }');
    opt.rightItem &&
      opt.rightItemImg &&
      (backgroundText +=
        '.m-marquee.' +
        opt.marqueeId +
        '.m-marquee-right .m-marquee-right-item:before { background-image: url("' +
        opt.rightItemImg +
        '"); }');

    backgroundText &&
      opt.$c.$marqueeC
        .addClass(opt.marqueeId)
        .append('<style>' + backgroundText + '</style>');
  }
  ...
  function extendOptions(opt, innerOpt) {
    ...
    opt.list =
      Object.prototype.toString.call(opt.list) === '[object Array]'
        ? opt.list
        : [];
